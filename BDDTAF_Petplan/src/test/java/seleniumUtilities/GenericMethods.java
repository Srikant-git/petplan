...
public class GenericMethods extends BaseRunner {
	...
	protected boolean assertElementPresent(By by, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				reporter.failureReport("CheckPoint :: ", locatorName + " is not present in the application", driver);
				// return false;
			} else {
...
//				reporter.SuccessReport("Visible of element is true :: ", "Element :: " + locatorName + "  is visible");
	...
	protected boolean verifyTextMatching(By by, String text, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				reporter.failureReport("CheckPoint :: ", text + " is not present  : " + locatorName,
						driver);
				// return false;
			} else {
	...
	protected boolean assertTextMatchingWithAttribute(By by, String text, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				reporter.failureReport("Verify : ", text + " is not present" + locatorName, driver);
			} else {
				reporter.SuccessReport("Verify : ", text + " is  present" + locatorName);
			}
		}
	}
	...
	protected boolean verifyTextEquals(String actText, String expText) throws Throwable {
		...
		try {
			...
			if (!flag) {
				...
				reporter.failureReport("CheckPoint:: ", "Expected Text: " + expText + "Actual Text: " + actText + " are not Maching", driver);
			} else {

				reporter.SuccessReport("CheckPoint:: ", "Expected Text: " + expText + "Actual Text: " + actText + " are Maching");
			}
		}
	}
		...
		try {
			...
			if (expected == false) {
				if (!status) {
					...
					reporter.failureReport("CheckPoint :: ", "Element is not displayed: " + locatorName + " : false", driver);
				}
			}
	...
	protected String getTextAttributeByValue(By locator, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				reporter.SuccessReport("GetAttribute :: " + locatorName,
						"Locator is Visible and attribute value of:: " + locatorName + " is Null");
...
//			wait.until(ExpectedConditions.elementToBeClickable(locator));
	...
	protected boolean moveToElementAndClick(By locator, String locatorName) throws Throwable {
		...
		try {
			...
			if (!flag) {
				reporter.failureReport("Move to Element And Click :: ", "Click action is not perform on ::" + locatorName, driver);
			} else {
	 ...
	 * @throws Throwable the throwable
	 */
	protected String getTextFromPDF(String pdfFilePath) throws Throwable {
...
//		// Get JS is Ready
//		boolean jsReady = ((JavascriptExecutor) driver).executeScript("return document.readyState").toString()
//				.equals("complete");
...
//	}
	...
	protected void validateCheckPointContains(By by, String expected, String locatorName) throws Throwable {
		...
		try {
			...
			if (ActualText.contains(expected)) {
				reporter.SuccessReportWithScreenshot("CheckPoint :: ", expected + " is  present  : " + locatorName,driver);
				Assert.assertTrue(true);
				flag = true;
			} else {
				reporter.failureReport("CheckPoint :: ", expected + " is not present  : " + locatorName,
						driver);
	...
	protected void validateCheckPointEquals(String actual, String expected, String locatorName) throws Throwable {
		...
		try {
			...
			if(flag) {
			reporter.SuccessReport("CheckPoint :: "+locatorName, "Actual : "+actual + " and Expected : " + expected + " value for "+locatorName+" are same.");
			}
			else {
			reporter.failureReport("CheckPoint :: "+locatorName, "Actual : "+actual + " and Expected : " + expected + " value for "+locatorName+" are not same.", driver);
			}
	 ...
	 * @throws Throwable the throwable
	 */
	public void pageRefreshButton() throws Throwable {
		try {			
			driver.getKeyboard().sendKeys(Keys.F5);
			reporter.SuccessReport("Page Referesh", "Successfully refereshed the current page");
		} catch (Exception e) {
			reporter.failureReport("Page Referesh", "Something went wrong while doing page refresh.");
		}

	}
	
	/*
	 * setEncryptedPassword -- Set encrypted password
	 * work
	 *
	 * @param locator     of (By)
	 * @param testData    of (String)
	 ...
	 * @throws Throwable the throwable
	 */
	/*public void setEncryptedPassword(By locator, String password, String locatorName) throws Throwable {

		boolean status = false;
		try {
			LOG.info("Class name : " + getCallerClassName() + "Method name : " + getCallerMethodName());
			LOG.info("Method : Type  ::  Locator : " + locatorName + " :: Data :" + password);
			WebDriverWait wait = new WebDriverWait(driver, 15);
			wait.until(ExpectedConditions.visibilityOfElementLocated(locator));
			LOG.info("Locator is Visible :: " + locator);
			driver.findElement(locator).clear();
			LOG.info("Cleared the existing Locator data : ");
			//String decryptPassword = decryptdata.DecryptDataValue(password);
			driver.findElement(locator).sendKeys(decryptPassword);
			status = true;
		} catch (Exception e) {
			status = false;
			LOG.info(e.getMessage());
			...
			if (!status) {
				if (reportIndicator) {
					reporter.failureReport("Enter text in :: " + locatorName, msgTypeFailure + password, driver);
				}
			} else {
				reporter.SuccessReport("Enter text in :: " + locatorName, msgTypeSuccess + password);
			}
			reportIndicator = true;
		}
	}*/
	...
	/*
	 * validateTextUsingContains
	 * work
	 *
	 * @param 
	 * @param Actual    of (String)
	 * @param Expected of (String)
	 * @return assertion
	 * @throws Throwable the throwable
	 */
	public void validateTextUsingContains(String actual,String expected) throws Throwable {
		...
		try {
			...
			LOG.info("Actual and expected values dosen't match, so failing the test."+"Expected Val: "+expected);
		}
	 ...
	 *
	 * @param 
	 * @param DirectoryName    of (String)
	 * @param DrivePath of (String)
	 * @return 
	 * @throws Throwable the throwable
	 */
	public void CreateRuntimeDirectory(String dirName,String dirpath) throws Throwable {
